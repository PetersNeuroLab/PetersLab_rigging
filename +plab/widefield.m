...
% Set up preview image (two colors)
n_axes = 2;
...
for curr_axes = 1:n_axes
    ...
    im_preview(curr_axes) = image(zeros(video_object.VideoResolution));
    axes(im_axes(curr_axes)); axis equal tight off;
end

metadata_text = uicontrol('Style','text','String','Frame metadata', ...
...
% Update status text
update_status_text(gui_data.status_text_h,'Ending recording (5 sec)');
...
% Enable buttons
set(gui_data.controls_h(gui_data.controls_h ~= obj),'Enable','on');
...
clim(gui_data.im_axes,[0,user_clim]);
end
