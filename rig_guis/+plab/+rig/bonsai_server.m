    ...
    methods (Access = protected)
        ...
        function obj = read_expcontrol_message(obj,source,eventdata)
            ...
            if strcmp(incoming_message, 'stop')
                ...
                % (Bonsai communication in OSC format: [path ,type message],
                % each component terminated with 0's to length multiple 4)
                osc_path = '/stop';
                osc_type = 's';
                osc_message = 'stop';

                osc_padstr = @(x) [x,zeros(1,4-mod(length(x),4))];
                bonsai_osc_stop_command = horzcat(osc_padstr(osc_path), ...
                    osc_padstr([',',osc_type]),osc_padstr(osc_message));
        ...
        function read_bonsai_message(obj,source,eventdata,local_save_path)
            ...
            if contains(incoming_message,'stop')
                ...
                % Check/wait for Bonsai process to finish
                bonsai_running = true;
                while bonsai_running
                    [status,cmdout] = system( ...
                        'tasklist /FI "IMAGENAME eq Bonsai.exe" | find /c "Bonsai"');
                    bonsai_process_find = str2num(cmdout);
                    if bonsai_process_find == 0
                        bonsai_running = false;
                    else
                        pause(1);
                    end
                end
        ...
        function move_data_to_server(obj,local_save_path)
            ...
            if status
                ...
                fprintf('BONSAI SERVER: error moving data to server: \n--> %s\n',message);
            end
