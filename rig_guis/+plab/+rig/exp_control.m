...
gui_fig = uifigure('Name','Experiment controller', ...
    'units','centimeters','position',[1,2,10,25], ...
...
% Set up recording notes
gui_data.handles.notes_recording.Enable = true;
...
oe_processors = webread(sprintf('http://%s:%d/api/processors', ...
    gui_data.ephys,plab.locations.ephys_port));
oe_processor_probenames = ...
    regexp({oe_processors.processors(1).streams.name}, ...
    'Probe[A-Z]','match','once');
oe_probes = unique(oe_processor_probenames( ...
    ~cellfun(@isempty,oe_processor_probenames)));
...
function update_notes(source,eventdata,gui_fig)
...
% Update day notes
if source == gui_data.handles.notes_day
    % (create recording path if it doesn't exist)
    if ~exist(rec_path,'dir')
        mkdir(rec_path)
    end
    notes_day_fn = fullfile(rec_path,join(["notes",mouse,rec_day],"_")) + ".txt";
    writelines(string(source.Value),notes_day_fn);
end

% Update recording notes
if source == gui_data.handles.notes_recording
    rec_time = source.UserData;
    notes_recording_fn = fullfile( ...
        plab.locations.filename('server',mouse,rec_day,rec_time), ...
        join(["notes",mouse,rec_day,rec_time],"_") + ".txt");
    writelines(string(source.Value),notes_recording_fn);
end
